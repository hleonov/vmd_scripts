# Water connectivity:
# Writes for every frame if the pore has water connectivity in<-->out from given residue, may include N atoms
# Usage example: vmd -dispdev text -e ~/vmd_scripts/water_connectivity.tcl -args system_from_PR2.pdb system_from_MD.xtc outfile HISH skip radius
...
#check_for_help "Usage example: vmd -dispdev text -e ~/vmd_scripts/water_connectivity.tcl -args <pdb> <xtc/trr file> <output> <resname> <skip_fr> <cutoff> <trj-type=xtc/trr> "
...
# protein center +- 20A reaches bulk
set z_thresh 20	
...
proc analyze_trj {fr} {
	...
	#center protein at {0,0,0}, so that bulk is at +- $z_thresh
	center_frame_by_selection "protein" $fr

	#initial selection to enter while loop
	...
	while {([llength $close_waters] > 0) && ($plus<1 || $minus<1)} {
		...
		foreach water $close_waters {
			set bulk 0
			#get index of current examined water
    		set i [lsearch $close_waters $water]

			#if current water is not already in global list - check if it reached bulk.
