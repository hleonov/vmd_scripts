...
proc order { frame } {
    ...
    for { set is 0 } {$is < 9} {incr is} {
        if {$is == 0} {
           set rmin [ expr $is + 0.1]
           set rmax [ expr $rmin + 4 - 0.1 ]
        } else {
           set rmin $rmax
           set rmax [ expr $rmin + 3]
        }
	...
	for { set i 2 } { $i < 19 } { incr i } {
	    set od1($i) 0.0
	    if {$i == 9 || $i == 10} { 
		set c [ atomselect top "resname POPE and (name C2${i} H${i}1 ) and 
                        (same residue as name C21 and z > 0 and 
		...
		set c [ atomselect top "resname POPE and (name C2${i} H${i}R H${i}S H${i}T) and 
                        (same residue as name C21 and z > 0 and 
                        (not within $rmin of protein or within $rmin of resname ETAM) and 
                        (within $rmax of protein or within $rmax of resname ETAM))" frame $frame ]
            puts "[$c num] $is"
	    } else { 
		set c [ atomselect top "resname POPE and (name C2${i} H${i}R H${i}S ) and 
                        (same residue as name C21 and z > 0 and 
                        (not within $rmin of protein or within $rmin of resname ETAM) and 
                        (within $rmax of protein or within $rmax of resname ETAM))" frame $frame ]
            puts "[$c num] $is"
	...
	for { set i 2 } { $i < 19 } { incr i } {
		...
		for { set j 0 } { $j < [expr [llength $idc] / 3 ] } { incr j } {
		    ...
		    set o2      [ lindex $r2 2]
		    set o2      [ expr 1.5 * ($o2 * $o2) - 0.5 ]
		    set od2($i) [ expr $od2($i) + ( 0.5 * ($o1 + $o2) ) ] 
		}
	    }
		...
		for { set j 0 } { $j < [expr [llength $idc] / 4 ] } { incr j } {
		    ...
		    set o3      [ lindex $r3 2 ]
		    set o3      [ expr 1.5 * ($o3 * $o3) - 0.5 ]
		    set od2($i) [ expr $od2($i) + ( 0.33 * ( $o1 + $o2 + $o3) ) ]
		}
	    }
	}
	
	# write data for monolayer 2 leg 1 to file 2
        
	for { set id 2 } { $id  < 19 } { incr id } {
	    if {$id == 9 || $id == 10} {
		puts $fid2($is) "[expr $od2($id) / ( [llength $idc] / 2 ) ]"
		flush $fid2($is)
	    } elseif {$id == 18} {
		puts $fid2($is) "[expr $od2($id) / ( [llength $idc] / 4 ) ]"
		flush $fid2($is)
	    } else {
		puts $fid2($is) "[expr $od2($id) / ( [llength $idc] / 3 ) ]"
		flush $fid2($is)
	    }
	}  
	...
	# write data for monolayer 2 leg 2 to file 
	
	for { set id 2 } { $id  < 17 } { incr id } {
	    if {$id != 17} {
		puts $fid4($is) "[expr $od1($id) / ( [llength $idc] / 3 ) ]"
		flush $fid4($is)
	    } else {
		puts $fid4($is) "[expr $od1($id) / ( [llength $idc] / 4 ) ]"
		flush $fid4($is)
	    }
	}
    }   
...
for { set is 0 } {$is < 9} {incr is} {
    ...
    set name3 "ord_${sys}_rad_c16_1_${is}.out"
    set fid3($is) [open $name3 w+]    
    set name4 "ord_${sys}_rad_c16_2_${is}.out"
    set fid4($is) [open $name4 w+]
}
